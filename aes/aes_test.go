...
func TestAddRoundKey(t *testing.T) {
  ...
  result_int := [16]uint32{}
  for i := uint32(0); i < 16; i++ {
    result_int[i] = state[i] ^key[i] 
  }
  right_result_int := []uint32{
    0xEB, 0x59, 0x8B, 0x1B,
    0x40, 0x2E, 0xA1, 0xC3,
    0xF2, 0x38, 0x13, 0x42,
    0x1E, 0x84, 0xE7, 0xD6,
	...
	result := fmt.Sprintf("%v", result_int)
	right_result := fmt.Sprintf("%v", right_result_int)
  if right_result != result {
		t.Errorf("Wrong addRoundKey %v != %v", result, right_result)

...
func TestOFB(t *testing.T) {
  ...
  iv := []byte{
		0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
		0xab, 0xf7, 0x15, 0x42, 0x19, 0xcf, 0x41, 0x3c,
  }
  ciphertext := ofb(block, iv, key, 2)
  plaintext := ofb(ciphertext, iv, key, 2)

	plain_candidate := fmt.Sprintf("%v", plaintext)
	plain_right := fmt.Sprintf("%v", block)

	if plain_candidate != plain_right {
		t.Errorf("Wrong key %v != %v", plain_candidate, plain_right)
  }
